...
static const uint8_t report_descriptor[] = {
    TUD_HID_REPORT_87_KEYBOARD(HID_REPORT_ID(HID_REPORT_ID_KEYBOARD))
};
...
void USBHIDKeyboard::begin(){
    keys_all.modifier = 0x00;
    keys_all.reserved = 0x00;
    for (int i=0; i<60; i++) {
...
void USBHIDKeyboard::sendReport(KeyReport* keys)
{
    ...
    if (keys->keys) {
        memcpy(report.keycode, keys->keys, 60);
    } else {
        memset(report.keycode, 0, 60);
    }
...
void USBHIDKeyboard::sendKey() {
    hid.SendReport(HID_REPORT_ID_KEYBOARD, &keys_all, sizeof(keys_all));
}
...
size_t USBHIDKeyboard::hidRaw(hid_keyboard_report60_t report) 
{
    return hid.SendReport(HID_REPORT_ID_KEYBOARD, &report, sizeof(report));
}
